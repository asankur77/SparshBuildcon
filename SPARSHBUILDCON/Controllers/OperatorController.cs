...
namespace SPARSHBUILDCON.Controllers
{
    public class OperatorController : Controller
    {
        ...
        public static string ptype, Receiptno = string.Empty;
      ...
      public  string gid()
        ...
        public void SaveVoucher(string newagentid, int month, int year, DateTime paymentdate)
        {
            ...
            wt.agentid = br.newagentid;
            wt.Credit = netamount;
            wt.netamount = total + netamount;
            wt.Debit = 0;
            wt.Date_Time = paymentdate.Date;
            wt.Sender_Id = User.Identity.Name;
            wt.Sender_Name = bn.OperatorName;
            wt.status = 1;
            wt.Mode = "Voucher";
            wt.Type = "Credit";
            ...
            wt.Remark = "Voucher Of month " + monthname;
        ...
        public JsonResult AutoCompleteintro(string term)
        {
            var list1 = db.AgentDetails.Where(x => x.Status == 0).ToList();
            var list = (from r in list1 where r.NewAgentId.ToLower().Contains(term.ToLower()) || r.name.ToLower().Contains(term.ToLower()) select new { r.NewAgentId, r.name }).Distinct();
            return Json(list, JsonRequestBehavior.AllowGet);
        }

        public JsonResult checkposition(string introid, int position = 0)
        {
            var intronewid = db.AgentDetails.Single(b => b.NewAgentId == introid);

            var status = 0;

            if (position == 1)
            {
                var count = db.binarys.Where(a => a.Id == intronewid.uid && a.lperson == 1).Count();
            ...
            {
                var count = db.binarys.Where(a => a.Id == intronewid.uid && a.rperson == 1).Count();
                if (count == 0)
                {
                    status = 0;
                }
        ...
        public JsonResult AutoCompleteintroid(string term)
        {
        ...
        {
            List<downlinename> temmeber = new List<downlinename>();
            SqlConnection con = new SqlConnection();
            ...
            cmd.Connection = con;
            con.Open();
            cmd.CommandText = "select * from AgentDetail where NewAgentId LIKE '" + introid + "%' OR NewAgentId LIKE '%" + introid + "%' OR  name LIKE '" + introid + "%' OR name LIKE '%" + introid + "%' order by NewAgentId";
            ...
            try
            {
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    {
                        temmeber.Add(new downlinename
                        {
                            memberid = item["NewAgentId"].ToString(),
            ...
            var list = (from j in temmeber where j.memberid.StartsWith(introid.ToLower()) || j.memberid.ToLower().Contains(introid.ToLower()) || j.username.ToLower().Contains(introid.ToLower()) select new { j.username, j.memberid }).Distinct();
            return Json(list, JsonRequestBehavior.AllowGet);
        }
        ...
        public ActionResult Member()
        {
            ...
            else
            {
                ...
                var clcount = (from c in db.ClosingMatchingTabs where c.date == maxdate && c.opid == User.Identity.Name && c.pagename == "Member" select c).Count();
                int dcount = (from a in db.DailyDepositTabs where a.opid == User.Identity.Name && a.trid == trid && a.date == maxdate select a).Count();
                if (dcount > 0)
                {
                    if (clcount == 0)
                    {
                        TempData["pagename"] = "Member";
        ...
        public ActionResult Member(Member_tab model)
        {
            ...
            {
                var ds = db.Blockdates.Where(c => c.date == model.Cdate && c.branchcode == model.BranchCode && c.status == 0).Count();
                if (ds > 0)
                {
                ...
                else
                {
                    ...
                    var br = db.Branchtabs.Single(b => b.BranchCode == row1.BranchCode);
                    ...
                    var newmemberid = string.Empty;
                    int bwcount = (from a in db.Member_tabs select a).Count() + 1;

                    if (bwcount < 10)
                    {
                        newmemberid = "M00000000" + bwcount;
                    }
                    else if (bwcount < 100 && bwcount >= 10)
                    {
                        newmemberid = "M0000000" + bwcount;
                    }
                    else if (bwcount < 1000 && bwcount >= 100)
                    {
                        newmemberid = "M000000" + bwcount;
                    }
                    else if (bwcount < 10000 && bwcount >= 1000)
                    {
                        newmemberid = "M00000" + bwcount;
                    }
                    else if (bwcount < 100000 && bwcount >= 10000)
                    {
                        newmemberid = "M0000" + bwcount;
                    }
                    else if (bwcount < 1000000 && bwcount >= 100000)
                    {
                        newmemberid = "M000" + bwcount;
                    }
                    else if (bwcount < 10000000 && bwcount >= 1000000)
                    {
                        newmemberid = "M00" + bwcount;
                    }
                    else if (bwcount < 100000000 && bwcount >= 10000000)
                    {
                        newmemberid = "M0" + bwcount;
                    }
                    else
                    {
                        newmemberid = "M" + bwcount;
                    }
                    ...
                    else
                    {
                        ...
                        r.MemberId = max;
                        r.NewMemberId = newmemberid;
                        r.BranchCode = row1.BranchCode;
                        r.MemberName = model.MemberName;
                        r.Address = model.Address;
                        r.Cdate = model.Cdate;
                        r.DOB = model.DOB;
                        r.Fee = model.Fee;
                        r.Gender = model.Gender;
                        r.Relationof = model.Relationof;
                        r.Father = model.Father;
                        r.Mother = model.Mother;
                        r.District = model.District;
                        r.Nationality = model.Nationality;
                        r.NomineeName = model.NomineeName;
                        r.NomineeRel = model.NomineeRel;
                        r.NomineeAge = model.NomineeAge;
                        r.Nomineeaddr = model.Nomineeaddr;
                        r.Share = model.Share;
                        r.District = model.District;
                        r.Type = model.Type;
                        r.Status = model.Status;
                        r.state = model.state;
                        r.Opid = User.Identity.Name;
                        r.Pin = model.Pin;
                        r.Mobile = model.Mobile;
                        r.panno = model.panno;
                        r.bankname = model.bankname;
                        r.accountno = model.accountno;
                        r.IFSC = model.IFSC;
                        db.Member_tabs.Add(r);
                        db.SaveChanges();
                ...
                switch (command)
                {
                    ...
                    case "Submit":
                        ...
                        if (count == 0)
                        {
                            ...
                            db.SaveChanges();
                            MyClass.Sendmsg(model.mobileno, "Dear " + model.acholdername + ", Your A/c no. " + aa + " credited with INR " + openingamount + " is successfully in " + csrt.CompanyName + " Visit us www.sparshbuildcon.org");
                            if (model.mobileno2 != "" || model.mobileno2 != null)
                            {
                                MyClass.Sendmsg(model.mobileno2, "Dear " + model.acholdername2 + ", Your A/c no. " + aa + " credited with INR " + openingamount + " is successfully in " + csrt.CompanyName + " Visit us www.sparshbuildcon.org");
                            }
        ...
        [HttpPost]
        public ActionResult NewAgent(AgentDetail model, HttpPostedFileBase Photo, string command, string memberid)
        {
            List<Member_tab> mt = new List<Member_tab>();
            if (!User.Identity.IsAuthenticated)
            {
        ...
        public ActionResult NewAgreement()
        {
            ...
            else
            {
                ...
                int trid = (from a in db.DailyDepositTabs where a.opid == User.Identity.Name && a.date == maxdate1 select a.trid).DefaultIfEmpty().Max();
                DateTime maxdate = (from a in db.DailyDepositTabs where a.opid == User.Identity.Name && a.trid == trid select a.date).DefaultIfEmpty().Max();
                var clcount = (from c in db.ClosingMatchingTabs where c.date == maxdate && c.opid == User.Identity.Name && c.pagename == "NewAgreement" select c).Count();
                 int dcount = (from a in db.DailyDepositTabs where a.opid == User.Identity.Name && a.trid == trid && a.date == maxdate select a).Count();
                 if (dcount > 0)
                 {
                     if (clcount == 0)
                     {
                         TempData["pagename"] = "NewAgreement";
                         return RedirectToAction("MatchingDailyClosing", "Operator");
                     }
                 }
                return View(mt);
                ...
                switch (command)
                {
                    ...
                    case "Submit":
                        ...
                        else
                        {
                            ...
                            if (bondid < 10)
                            {
                                newbondid = "SBC0010000" + bondid;
                            }
                            else if (bondid < 100 && bondid >= 10)
                            {
                                newbondid = "SBC001000" + bondid;
                            }
                            else if (bondid < 1000 && bondid >= 100)
                            {
                                newbondid = "SBC00100" + bondid;
                            }
                            else if (bondid < 10000 && bondid >= 1000)
                            {
                                newbondid = "SBC0010" + bondid;
                            }
                            else
                            {
                                newbondid = "SBC001" + bondid;
                            }

                            var adr = db.AgentDetails.Single(ad => ad.NewAgentId == bd.newintroducerid);

                            ProTerm pr = db.ProTerms.Single(p => p.projectid == bd.projectid && p.term == bd.term);
                            ...
                            //var row = db.Member_tabs.Single(ui => ui.NewMemberId == bd.);
                            ...
                            if (bd.paymethod != "Cheque" && bd.paymethod != "banktransaction" && bd.discountper == 0)
                            {
                                ...
                                bd.expectedraisablevalue = bd.totalcon;
                                bd.downpayment = 0;//
                                ...
                                var ccount = db.appltabs.Where(c => c.newintroducerid == bd.newintroducerid).Count();
                                if (ccount == 1 && adr.formfee == 0)
                                {
                                    adr.Status = 1;
                                    adr.activationdate = bd.formdate;
                                    db.SaveChanges();
                                }
                                ...
                                try
                                {
                                    con.Open();
                                    cmd.ExecuteNonQuery();
                                    //MyClass.Sendmsg(bd.mobileno, "Dear " + bd.name + ", your Loginid:" + newbondid + " Password:" + bd.mobileno + " For your A/c details please visit www.sparshbuildcon.org");
                                    //MyClass.Sendmsg(bd.mobileno, "Dear Customer your Application no- " + newbondid + " has been activated with Rs. " + bd.bookingamount + " in the plan " + pt.Planname + "Thank you for your kind believe us for help contact. SPARSH BUILDCON");
                                    ...
                                    Response.Write("<script>alert('" + newbondid + " " + mr.custname + " created Successfully....')</script>");
                                    ViewBag.msg = newbondid + " " + mr.custname + " created Successfully....";
                                    NewBondid = newbondid;
                            ...
                            else
                            {
                                ...
                                td.expectedraisablevalue = tbd.totalcon;
                                ...
                                td.paymenttype = tbd.paymenttype;
                                td.propertytype = tbd.propertytype;
                                td.propertypreference = tbd.propertypreference;
                                td.incomegroup = tbd.incomegroup;
                                td.plotdesp = tbd.plotdesp;
                    ...
                    case "Submit":
                        ...
                        else
                        {
                            ...
                            try
                            {
                                con.Open();
                                cmd.ExecuteNonQuery();
                                MyClass.Sendmsg(model.mobileno, "Dear " + model.name + ", your Loginid:" + newbondid + " Password:" + model.mobileno + " For your A/c details please visit www.sparshbuildcon.org");
                                MyClass.Sendmsg(model.mobileno, "Dear Customer your Application no- " + newbondid + " has been activated with Rs. " + payment + " in the plan " + model.planname + "Thank you for your kind believe us for help contact. SPARSH BUILDCON");
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                try
                                {
                                    ...
                                    cmd.ExecuteNonQuery();
                                    con.Close();
                                    // MyClass.Sendmsg(model.mobileno, "Dear " + model.name + ", your Loginid:" + model.newbondid + " Password:" + model.mobileno + " For your A/c details please visit www.sparshbuildcon.org");
                                    // MyClass.Sendmsg(model.mobileno, "Dear Customer your Application no- " + model.newbondid + " has been activated with Rs. " + model.payment + " in the plan " + pr.Planname + "Thank you for your kind believe us for help contact. SPARSH BUILDCON");
                                    ViewBag.msg = mr.custname + " Created Successfully....";
                                    ViewData["newbondid"] = model.newbondid;
        ...
        public ActionResult PrintBookingReceipt(string newbondid)
        {
            ...
            else
            {
                if (newbondid != null)
                {
                    ...
                    var amountinwords = n.changeToWords(st.bookingamount.ToString(), true);
                    int from = (from p in db.Installmenttabs where p.newbondid == newbondid && p.paymentno == 1 select p.installmentno).Min();
                    int to = (from p in db.Installmenttabs where p.newbondid == newbondid && p.paymentno == 1 select p.installmentno).Max();
                    ...
                    cmd.Parameters.AddWithValue("@newbondid", newbondid);
                    cmd.Parameters.AddWithValue("@paymentno", 1);
                    cmd.Parameters.AddWithValue("@opid", User.Identity.Name);
                    cmd.Parameters.AddWithValue("@payamount", st.bookingamount);
                    ...
                    try
                    {
                        ...
                        ReportDocument rd = new ReportDocument();
                        rd.Load(Path.Combine(Server.MapPath("~/Reports"), "BookingReceipt.rpt"));
        ...
        public ActionResult AddExpense()
        {
            ...
            else
            {
                ...
                int trid = (from a in db.DailyDepositTabs where a.opid == User.Identity.Name && a.date == maxdate1 select a.trid).DefaultIfEmpty().Max();
                DateTime maxdate = (from a in db.DailyDepositTabs where a.opid == User.Identity.Name && a.trid == trid select a.date).DefaultIfEmpty().Max();
                var clcount = (from c in db.ClosingMatchingTabs where c.date == maxdate && c.opid == User.Identity.Name && c.pagename == "AddExpense" select c).Count();
                 int dcount = (from a in db.DailyDepositTabs where a.opid == User.Identity.Name && a.trid == trid && a.date == maxdate select a).Count();
                 if (dcount > 0)
                 {
                     if (clcount == 0)
                     {
                         TempData["pagename"] = "AddExpense";
                         return RedirectToAction("MatchingDailyClosing", "Operator");
                     }
                 }
                return View();
        ...
        public ActionResult PaymentRenewal(TempInstallmenttab tp, HttpPostedFileBase Chequeimage, string newbondid2, string planname, Double term, string mode, Double latefine, int from, int to, string paymethod, DateTime pdate, string checkorddno, string drawno, Double relief, Double payamount, string amountinword, string trbank, string transactionid, string transactiondate, string tramount, string trpmethod, string holderacno)
        {
            ...
            {
                var tempcount = db.TempInstallmenttabs.Where(c => c.newbondid == newbondid2 && c.status != 2).Count();
                var br = db.Operators.Single(o => o.OperatorId == User.Identity.Name);
                    ...
                    if (tempcount == 0)
                    {
                        ...
                        else
                        {
                            ...
                            try
                            {
                                ...
                                ViewData["newbondid"] = newbondid2;
                                ViewData["method"] = paymethod;
                                    var tins = db.Installmenttabs.Single(a => a.newbondid == newbondid2 && a.installmentno == to);
                                ...
                                MyClass.Sendmsg(cr.mobileno, "Dear Customer " + cr.name + ", Your EMI of Rs." + payamount + " for the month " + tins.prevexpirydate.ToString("MMMMMMMMMM") + " for loan a/c no. " + cr.loanid + " vide " + paymethod + " " + checkorddno + " has been deposited successfully. Regards- " + cm.CompanyName);
        ...
        [HttpGet]
        public ActionResult SLPReturn(string newbondid, string receiptno)
        {
            if (!User.Identity.IsAuthenticated)
            {
                return RedirectToAction("Logout", "Operator");
            ...
            {
                List<misdetails> mislist = new List<misdetails>();
                if (newbondid != null)
                {
                    var br = db.appltabs.Single(b => b.newbondid == newbondid);
                    var ar = db.AgentDetails.Single(a => a.NewAgentId == br.newintroducerid);
                    NumberToEnglish nw = new NumberToEnglish();
                    MISInstallmenttab ir = db.MISInstallmenttabs.Single(i => i.newbondid == newbondid && i.receiptno == receiptno);
                    var amountinwords = nw.changeToWords(ir.amount.ToString(), true);
                    ir.payamount = ir.amount;
                    ir.paymentdate = DateTime.Now.Date;
                    ir.paymethod = "Transfer to SLP Wallet";
                    ...
                    Roiincome roi = new Roiincome();
                    roi.introid = br.introducerid;
                    roi.intronewid = br.newintroducerid;
                    roi.introname = ar.name;
                    roi.rs = ir.amount;
                    roi.date = DateTime.Now.Date.Day;
                    roi.month = DateTime.Now.Date.Month;
                    roi.year = DateTime.Now.Date.Year;
                    roi.status = 0;
                    roi.point = ir.installmentno;
                    roi.package = Convert.ToInt32(br.totalcon);
                    roi.nextsunday = DateTime.Now.Date;
                    roi.members = "NA";
                    roi.position = ar.position;
                    roi.custid = br.bondid;
                    roi.custnewid = br.newbondid;
                    roi.custname = br.name;
                    roi.paidstatus = 0;
                    roi.LastPaidDate = DateTime.Now;
                    db.Roiincomes.Add(roi);
                    db.SaveChanges();

                    MyClass.Sendmsg(br.mobileno, "Dear " + br.name + ",Your SLP Return of INR " + ir.amount + " is transfer to SLP wallet for Booking Id: " + newbondid + " on date: " + DateTime.Now.Date.ToString("dd/MM/yyyy"));

                    var rclist = db.MISInstallmenttabs.Where(c => c.newbondid == newbondid).ToList();
                    foreach (var rc in rclist)
                    {
                        if (rc.payamount > 0)
                        {
                            mislist.Add(new misdetails { newbondid = br.newbondid, name = br.name, receiptno = rc.receiptno, amount = rc.amount, installmentno = rc.installmentno, duedate = rc.prevexpirydate, expirydate = rc.expirydate, status = 1 });
                        }
                        else
                        {
                            mislist.Add(new misdetails { newbondid = br.newbondid, name = br.name, receiptno = rc.receiptno, amount = rc.amount, installmentno = rc.installmentno, duedate = rc.prevexpirydate, expirydate = rc.expirydate, status = 0 });
                        }
                    }
                }
                NewBondid = newbondid;
                Receiptno = receiptno;
                return View(mislist.OrderBy(i => i.installmentno));
            }
        }
        [HttpPost]
        public ActionResult SLPReturn(string newbondid)
        {
            if (!User.Identity.IsAuthenticated)
            {
                return RedirectToAction("Logout", "Operator");
        ...
        public ActionResult PrintCheque(string newbondid, string receiptno)
        {
            ...
            else
            {
                ...
                var s = db.MISInstallmenttabs.Single(i => i.newbondid == newbondid && i.receiptno == receiptno);
                var branch = db.Branchtabs.Single(b => b.BranchCode == app.branchcode);
                var install = (from a in db.MISInstallmenttabs where a.newbondid == newbondid select a).Count();
                var tt = (from a in db.MISInstallmenttabs where a.newbondid == newbondid && a.payamount != 0 select a.amount).Sum();
                string sdt = s.expirydate.ToString("dd/MM/yyyy");
                NumberToEnglish ne = new NumberToEnglish();
                var amountword = ne.changeToWords(s.amount.ToString(), true);
                var vcno = branch.BranchCode + "-" + app.bondid + "-" + app.newbondid + "-" + (app.term).ToString();
                if (!string.IsNullOrWhiteSpace(app.checkorddno))
                {
                    payamt = s.amount.ToString();
                    chkno = s.chekddno;
                }
                else
                {
                ...
                double roi = 0;
                if (app.totalcon >= 5000 && app.totalcon < 100000)
                {
                    roi = 5.5;
                }
                else if (app.totalcon >= 100000 && app.totalcon < 300000)
                {
                    roi = 6;
                }
                else if (app.totalcon >= 300000 && app.totalcon < 1500000)
                {
                    roi = 7;
                }
                else if (app.totalcon >= 1500000)
                {
                    roi = 8;
                }
                print.Add(new Printcheckbook { insatallmentno = s.installmentno.ToString() + "/" + install.ToString(), vcno = vcno, intrestamt = s.amount, dateofmaturity = app.expirydate.Date, maturityamt = tt.ToString(), voucherdate = Convert.ToDateTime(s.paymentdate).Date, roi = roi.ToString(), brancode = app.branchcode, branchadd = branch.branchaddress, Name = app.name, Tranid = s.receiptno, Amount = app.totalcon, term = s.term_plan.ToString() + " " + "Year", Planname = s.planname, AccountNo = s.newbondid, Amountword = amountword, date = sdt, chequeno = chkno, chequeamount = payamt, openingdate = app.formdate, closingdate = app.expirydate, instano = s.installmentno, age = app.age, father = app.fathername, relation = app.plantype + " " + "Only", address = app.addr });
            }
            ...
            else
            {
                if (newbondid != null)
                {
                    ...
                    try
                    {
                        ...
                        ReportDocument rd = new ReportDocument();
                        rd.Load(Path.Combine(Server.MapPath("~/Reports"), "PaymentReceipt.rpt"));
                        rd.SetDataSource(drlist);
            ...
            else
            {
                ...
                if (app.status == 1)
                {
                    ...
                    foreach (var s in ReturnAmount)
                    {
                        ...
                        NumberToEnglish ne = new NumberToEnglish();
                        var amountword = ne.changeToWords(s.amount.ToString(), true);
                        print.Add(new Printcheckbook { Name = app.name, Tranid = s.receiptno, Amount = s.amount, term = s.term_plan.ToString(), Planname = s.planname, AccountNo = s.newbondid, Amountword = amountword, date = sdt });
                    }
                }
                else
                {
        ...
        public ActionResult BTree(string newid, string aa)
        {
            ...
            else
            {
                ...
                {
                    var log = db.NewLogins.Single(a => a.UserName == User.Identity.Name);
                    if (log.status == 1 || log.type == "Admin" || log.type == "Branch" || log.type == "Operator")
                    {
                        if (newid == null)
                        {
        ...
        public ActionResult Broker(string brokerid, int s = 0)
        {
            ...
            else
            {
                ...
                {
                    var br = db.Operators.Single(o => o.OperatorId == User.Identity.Name);
                    var dr = (from o in db.AgentDetails where o.BranchCode == br.BranchCode orderby o.NewAgentId select o).ToList();
        ...
        public ActionResult PrintBrokerList()
        {
            ...
            var cr = db.CompanyInfos.Single(c => c.Id == 1);
            var brolist = (from o in db.AgentDetails where o.BranchCode == br.BranchCode orderby o.NewAgentId select o).ToList();
            ...
            ReportDocument rd = new ReportDocument();
            rd.Load(Path.Combine(Server.MapPath("~/Reports"), "NewBrokerlist.rpt"));
        ...
        public ActionResult DatewiseBroker(DateTime sdate, DateTime edate)
        {
            ...
            else
            {
                ...
                var blist = (from g in db.AgentDetails where g.BranchCode == pr.BranchCode && g.Doj >= sdate && g.Doj <= edate orderby g.NewAgentId select g).ToList();
                foreach (var b in blist)
                {
                    dwblist.Add(new BrokerList { newagentid = b.NewAgentId, name = b.name, rankname = b.formfee.ToString(), introducerid = b.NewIntroducerId, mobile = b.Mobile, doj = b.Doj, branchname = br.BranchName, sdate = sdate, edate = edate });
                }

                stdate = sdate;
                enddate = edate;
                return View(dwblist.OrderBy(o => o.doj));
        ...
        public ActionResult PrintDatewiseBroker()
        {
            ...
            else
            {
                ...
                var br = db.Branchtabs.Single(o => o.BranchCode == pr.BranchCode);
                var cr = db.CompanyInfos.Single(c => c.Id == 1);
                var blist = (from g in db.AgentDetails where g.BranchCode == pr.BranchCode && g.Doj >= stdate && g.Doj <= enddate orderby g.NewAgentId select g).ToList();
                foreach (var b in blist)
                {
                    dwblist.Add(new BrokerList { newagentid = b.NewAgentId, name = b.name, rankname = b.formfee.ToString(), introducerid = b.NewIntroducerId, mobile = b.Mobile, doj = b.Doj, branchname = br.BranchName, sdate = stdate, edate = enddate, companyname = cr.CompanyName, address = cr.Address, emailid = cr.Emailid, contact = cr.Contact, domainname = cr.HeadOffice });
                }

                ReportDocument rd = new ReportDocument();
                rd.Load(Path.Combine(Server.MapPath("~/Reports"), "NewDatewiseBroker.rpt"));
                rd.SetDataSource(dwblist.OrderBy(o => o.doj));
        ...
        public ActionResult BrokerPaymentDetail(string newagentid)
        {
            ...
            else
            {
                ...
                var bvlist = (from vt in db.Voucher_Reports where vt.agentid == ar.AgencyCode && vt.branchcode == br.BranchCode orderby vt.agentid select vt).ToList();
                foreach (var bv in bvlist)
                {
                    MonthName mn = new MonthName();
                    var monthname = mn.numbertomonthname(bv.month);
                    bplist.Add(new BrokerPayment { newagentid = newagentid, name = ar.name, Business = bv.bussiness, commission = bv.commission, tds = bv.tds, netamount = bv.netamount, month = monthname, year = bv.year, pdate = bv.date, branchname = br.BranchName, companyname = cr.CompanyName, branch = mr.branchname });
                }
                NewAgentid = newagentid;
                return View(bplist);
            }
        ...
        public ActionResult BrokerSelfCollection(string newagent, DateTime sdate, DateTime edate)
        {
            ...
            else
            {
                ...
                if (agcount > 0)
                {
                    ...
                    var tbd = (from ap in db.appltabs where ap.newintroducerid == newagent && ap.status == 1 select ap.bondid);
        ...
        public ActionResult PrintselfCollection()
        {
            ...
            if (agcount > 0)
            {
                ...
                var tbd = (from ap in db.appltabs where ap.newintroducerid == NewAgentid && ap.status == 1 select ap.bondid);
        ...
        public ActionResult Customer(int option)
        {
            ...
            else
            {
                ...
                {
                    custlist.Add(new DatewiseCustomerList { newbondid = cust.newbondid, name = cust.name, newintroducerid = cust.newintroducerid, planname = cust.planname, mode = cust.mode, payment = cust.payment, bookingamount = cust.bookingamount, downpayment = cust.downpayment, plotsize = cust.nolandunit, Mobile = cust.mobileno, formdate = cust.formdate, branchname = User.Identity.Name, companyname = cr.CompanyName, branch = mr.branchname, bond = mr.custname, plan = cust.paymethod, address = cr.Address, emailid = cr.Emailid, contact = cr.Contact, domainname = cr.HeadOffice });
                }
                PId = option;
        ...
        public ActionResult DatewiseCustomer(DateTime sdate, DateTime edate)
        {
            ...
            else
            {
                ...
                var tlist = (from cu in db.appltabs where cu.formdate >= sdate && cu.formdate <= edate && cu.opid == pr.OperatorId && cu.status == 1 select cu).ToList();
                foreach (var i in tlist)
                {
                    clist.Add(new DatewiseCustomerList { newbondid = i.newbondid, name = i.name, Mobile = i.mobileno, formdate = i.formdate, newintroducerid = i.newintroducerid, planname = i.planname, mode = i.mode, payment = i.payment, bookingamount = i.bookingamount, downpayment = i.downpayment, plotsize = i.nolandunit, branch = mr.branchname, companyname = cr.CompanyName, branchname = br.BranchName, plan = i.paymethod, bond = mr.custname, sdate = sdate, edate = edate, block = i.block, plotno = i.plotno });
                }
        ...
        public ActionResult PrintDatewiseCustomer()
        {
            ...
            else
            {
                ...
                var tlist = (from cu in db.appltabs where cu.formdate >= stdate && cu.formdate <= enddate && cu.opid == pr.OperatorId && cu.status == 1 select cu).ToList();

                foreach (var i in tlist)
                {
                    clist.Add(new DatewiseCustomerList { newbondid = i.newbondid, name = i.name, Mobile = i.mobileno, formdate = i.formdate, newintroducerid = i.newintroducerid, planname = i.planname, mode = i.mode, payment = i.payment, bookingamount = i.bookingamount, downpayment = i.downpayment, plotsize = i.nolandunit, branch = mr.branchname, companyname = cr.CompanyName, branchname = br.BranchName, plan = i.paymethod, bond = mr.custname, sdate = stdate, edate = enddate, block = i.block, plotno = i.plotno });
                }
        ...
        public ActionResult PrintExpanse()
        {
            ...
            comm = db.Expenses.Where(s => s.date_time >= ab && s.date_time <= b && s.branchcode == or.BranchCode).ToList();
            ReportDocument rd = new ReportDocument();
        ...
        public ActionResult CompleteDailyCollection(DateTime sdate, DateTime edate, string type)
        {
            ...
            else
            {
                ...
                var dipositsaving = (from j in db.TransactionTabs where j.pdate >= sdate && j.pdate <= edate && j.opid == pr.OperatorId select j.credit).DefaultIfEmpty(0).Sum();
                var widthdrawsaving = (from j in db.TransactionTabs where j.pdate >= sdate && j.pdate <= edate && j.opid == pr.OperatorId select j.debit).DefaultIfEmpty(0).Sum();
                totalvoucher = (from ag in db.Voucher_Reports where ag.date >= sdate && ag.date <= edate && ag.Operatorid == pr.OperatorId select ag.netamount).DefaultIfEmpty(0).Sum();
                totalexp = (from ag in db.Expenses where ag.date_time >= sdate && ag.date_time <= edate && ag.opid == pr.OperatorId select ag.amount).DefaultIfEmpty(0).Sum();
                appsum = (from b in db.appltabs where b.formdate >= sdate && b.formdate <= edate && b.opid == pr.OperatorId && b.paymethod == type && tbd.Contains(b.bondid) select b.applicationcharge).DefaultIfEmpty(0).Sum();
                formsum = (from ag in db.AgentDetails where ag.Doj >= sdate && ag.Doj <= edate && ag.operatorid == pr.OperatorId select ag.formfee).DefaultIfEmpty(0).Sum();
                memberfees = (from ag in db.Member_tabs where ag.Cdate >= sdate && ag.Cdate <= edate && ag.Opid == pr.OperatorId select ag.Fee).DefaultIfEmpty(0).Sum();
                acformfees = (from ag in db.SavingAccountInfos where ag.Doj >= sdate && ag.Doj <= edate && ag.opid == pr.OperatorId select ag.formfee).DefaultIfEmpty(0).Sum();

                var bd = (from p in db.Installmenttabs where p.paymentdate >= sdate && p.paymentdate <= edate && p.paymethod == type && p.paymentno != 0 && p.opid == pr.OperatorId && tbd.Contains(p.bondid) select new { p.bondid }).Distinct();
        ...
        public ActionResult PrintCompleteDailyCollection()
        {
            ...
            Double formsum = 0, appsum = 0, totalexp = 0, memberfees = 0, acformfees = 0, totalvoucher = 0;
            var pst = db.Plans.Single(c => c.Plancode == 4);
            var dipositsaving = (from j in db.TransactionTabs where j.pdate >= stdate && j.pdate <= enddate && j.opid == pr.OperatorId select j.credit).DefaultIfEmpty(0).Sum();
            var widthdrawsaving = (from j in db.TransactionTabs where j.pdate >= stdate && j.pdate <= enddate && j.opid == pr.OperatorId select j.debit).DefaultIfEmpty(0).Sum();
            totalvoucher = (from ag in db.Voucher_Reports where ag.date >= stdate && ag.date <= enddate && ag.Operatorid == pr.OperatorId select ag.netamount).DefaultIfEmpty(0).Sum();
            totalexp = (from ag in db.Expenses where ag.date_time >= stdate && ag.date_time <= enddate && ag.opid == pr.OperatorId select ag.amount).DefaultIfEmpty(0).Sum();
            appsum = (from b in db.appltabs where b.formdate >= stdate && b.formdate <= enddate && b.paymethod == typ && b.opid == pr.OperatorId && tbd.Contains(b.bondid) select b.applicationcharge).DefaultIfEmpty(0).Sum();
            formsum = (from ag in db.AgentDetails where ag.Doj >= stdate && ag.Doj <= enddate && ag.operatorid == pr.OperatorId select ag.formfee).DefaultIfEmpty(0).Sum();
            memberfees = (from ag in db.Member_tabs where ag.Cdate >= stdate && ag.Cdate <= enddate && ag.Opid == pr.OperatorId select ag.Fee).DefaultIfEmpty(0).Sum();
            acformfees = (from ag in db.SavingAccountInfos where ag.Doj >= stdate && ag.Doj <= enddate && ag.opid == pr.OperatorId select ag.formfee).DefaultIfEmpty(0).Sum();

            var bd = (from p in db.Installmenttabs where p.paymentdate >= stdate && p.paymentdate <= enddate && p.paymethod == typ && p.paymentno != 0 && p.opid == pr.OperatorId && tbd.Contains(p.bondid) select new { p.bondid }).Distinct();
        ...
        public ActionResult RPStatementReport(DateTime sdate, DateTime edate)
        {
            ...
            else
            {
                ...
                var todayfdcoll = (from df in db.Installmenttabs where df.paymentdate < sdate && df.mode == "Single" && df.type != "Adjustment" && df.opid == pr.OperatorId && tbd.Contains(df.bondid) select df.payamount).DefaultIfEmpty(0).Sum();
                var todayRdcoll = (from df in db.Installmenttabs where df.paymentdate < sdate && df.mode != "Single" && df.type != "Adjustment" && df.opid == pr.OperatorId && df.installmentno == 1 && df.planname == plan1.shortcutname && tbd.Contains(df.bondid) select df.payamount).DefaultIfEmpty(0).Sum();
                var bookingy = (from df in db.appltabs where df.formdate < sdate && df.type != "Adjustment" && tbd.Contains(df.bondid) && df.opid == pr.OperatorId select df.bookingamount).DefaultIfEmpty(0).Sum();
                var advancey = (from df in db.appltabs where df.formdate < sdate && df.type != "Adjustment" && tbd.Contains(df.bondid) && df.opid == pr.OperatorId select df.downpayment).DefaultIfEmpty(0).Sum();
                var todayNewBondcoll = todayfdcoll + todayRdcoll;
                var todayRenewalBondcoll = (from df in db.Installmenttabs where df.paymentdate < sdate && df.installmentno != 1 && df.type != "Adjustment" && df.opid == pr.OperatorId && tbd.Contains(df.bondid) select df.payamount).DefaultIfEmpty(0).Sum();

                var yeslatefee = (from df in db.Installmenttabs where df.paymentdate < sdate && df.type != "Adjustment" && df.opid == pr.OperatorId && tbd.Contains(df.bondid) select df.latefine).DefaultIfEmpty(0).Sum();
                var yesreleif = (from df in db.Installmenttabs where df.paymentdate < sdate && df.type != "Adjustment" && df.opid == pr.OperatorId && tbd.Contains(df.bondid) select df.reliefrs).DefaultIfEmpty(0).Sum();
                var ylatefee = yeslatefee - yesreleif;

                var bformfeey = (from df in db.AgentDetails where df.Doj < sdate && df.operatorid == pr.OperatorId select df.formfee).DefaultIfEmpty(0).Sum();
                var cformfeey = (from df in db.appltabs where df.formdate < sdate && df.type != "Adjustment" && df.opid == pr.OperatorId && tbd.Contains(df.bondid) select df.applicationcharge).DefaultIfEmpty(0).Sum();
                var mformfeey = (from df in db.Member_tabs where df.Cdate < sdate && df.Opid == pr.OperatorId select df.Fee).DefaultIfEmpty(0).Sum();
                var acformfeey = (from df in db.SavingAccountInfos where df.Doj < sdate && df.opid == pr.OperatorId select df.formfee).DefaultIfEmpty(0).Sum();
                var deposity = (from df in db.TransactionTabs where df.pdate < sdate && df.opid == pr.OperatorId select df.credit).DefaultIfEmpty(0).Sum();
                ...
                var AdvPayment = (from df in db.AdvBrokerPaymentTabs where df.date < sdate && df.opid == pr.OperatorId select df.amount).DefaultIfEmpty(0).Sum();
                var withdrawy = (from df in db.TransactionTabs where df.pdate < sdate && df.opid == pr.OperatorId select df.debit).DefaultIfEmpty(0).Sum();
                var expansey = (from df in db.Expenses where df.date_time < sdate && df.opid == pr.OperatorId select df.amount).DefaultIfEmpty(0).Sum();
                var todayVouchercoll = (from df in db.Voucher_Reports where df.date < sdate && df.Operatorid == pr.OperatorId select df.netamount).DefaultIfEmpty(0).Sum();
                var mat = (from df in db.MaturityTabs where df.maturitydate < sdate && df.opid == pr.OperatorId select df.maturityamount).DefaultIfEmpty(0).Sum();
                ...
                /////Today's Collection /////
                var todayfdcoll1 = (from df in db.Installmenttabs where df.paymentdate >= sdate && df.paymentdate <= edate && df.mode == "Single" && df.type != "Adjustment" && df.opid == pr.OperatorId && tbd.Contains(df.bondid) select df.payamount).DefaultIfEmpty(0).Sum();
                var todayRdcoll1 = (from df in db.Installmenttabs where df.paymentdate >= sdate && df.paymentdate <= edate && df.mode != "Single" && df.installmentno == 1 && df.type != "Adjustment" && df.opid == pr.OperatorId && df.planname == plan1.shortcutname && tbd.Contains(df.bondid) select df.payamount).DefaultIfEmpty(0).Sum();
                var todaybooking = (from df in db.appltabs where df.formdate >= sdate && df.formdate <= edate && df.type != "Adjustment" && tbd.Contains(df.bondid) && df.opid == pr.OperatorId select df.bookingamount).DefaultIfEmpty(0).Sum();
                var todayadvance = (from df in db.appltabs where df.formdate >= sdate && df.formdate <= edate && df.type != "Adjustment" && tbd.Contains(df.bondid) && df.opid == pr.OperatorId select df.downpayment).DefaultIfEmpty(0).Sum();
                var todayNewBondcoll1 = todayfdcoll1 + todayRdcoll1;
                var todayRenewalBondcoll1 = (from df in db.Installmenttabs where df.paymentdate >= sdate && df.paymentdate <= edate && df.installmentno != 1 && df.opid == pr.OperatorId && df.type != "Adjustment" && tbd.Contains(df.bondid) select df.payamount).DefaultIfEmpty(0).Sum();

                var todaylatefee = (from df in db.Installmenttabs where df.paymentdate >= sdate && df.paymentdate <= edate && df.type != "Adjustment" && df.opid == pr.OperatorId && tbd.Contains(df.bondid) select df.latefine).DefaultIfEmpty(0).Sum();
                var todayreleif = (from df in db.Installmenttabs where df.paymentdate >= sdate && df.paymentdate <= edate && df.type != "Adjustment" && df.opid == pr.OperatorId && tbd.Contains(df.bondid) select df.reliefrs).DefaultIfEmpty(0).Sum();
                var latefee = todaylatefee - todayreleif;

                var bformfee = (from df in db.AgentDetails where df.Doj >= sdate && df.Doj <= edate && df.operatorid == pr.OperatorId select df.formfee).DefaultIfEmpty(0).Sum();
                var cformfee = (from df in db.appltabs where df.formdate >= sdate && df.formdate <= edate && df.type != "Adjustment" && df.opid == pr.OperatorId && tbd.Contains(df.bondid) select df.applicationcharge).DefaultIfEmpty(0).Sum();
                var mformfee = (from df in db.Member_tabs where df.Cdate >= sdate && df.Cdate <= edate && df.Opid == pr.OperatorId select df.Fee).DefaultIfEmpty(0).Sum();
                var acformfee = (from df in db.SavingAccountInfos where df.Doj >= sdate && df.Doj <= edate && df.opid == pr.OperatorId select df.formfee).DefaultIfEmpty(0).Sum();
                var deposit = (from df in db.TransactionTabs where df.pdate >= sdate && df.pdate <= edate && df.opid == pr.OperatorId select df.credit).DefaultIfEmpty(0).Sum();
                ...
                var AdvPayment1 = (from df in db.AdvBrokerPaymentTabs where df.date >= sdate && df.date <= edate && df.opid == pr.OperatorId select df.amount).DefaultIfEmpty(0).Sum();
                var withdraw = (from df in db.TransactionTabs where df.pdate >= sdate && df.pdate <= edate && df.opid == pr.OperatorId select df.debit).DefaultIfEmpty(0).Sum();
                var expanse = (from df in db.Expenses where df.date_time >= sdate && df.date_time <= edate && df.opid == pr.OperatorId select df.amount).DefaultIfEmpty(0).Sum();
                var todayVouchercoll1 = (from df in db.Voucher_Reports where df.date >= sdate && df.date <= edate && df.Operatorid == pr.OperatorId select df.netamount).DefaultIfEmpty(0).Sum();
                var mat1 = (from df in db.MaturityTabs where df.maturitydate >= sdate && df.maturitydate <= edate && df.opid == pr.OperatorId select df.maturityamount).DefaultIfEmpty(0).Sum();
        ...
        public ActionResult PrintClosingBalance()
        {
            ...
            var tbd = (from ap in db.appltabs where ap.status == 1 select ap.bondid);
            /////Yesterday's Collection /////
            var todayfdcoll = (from df in db.Installmenttabs where df.paymentdate < stdate && df.mode == "Single" && df.type != "Adjustment" && df.opid == pr.OperatorId && tbd.Contains(df.bondid) select df.payamount).DefaultIfEmpty(0).Sum();
            var todayRdcoll = (from df in db.Installmenttabs where df.paymentdate < stdate && df.mode != "Single" && df.type != "Adjustment" && df.opid == pr.OperatorId && df.installmentno == 1 && df.planname == plan1.shortcutname && tbd.Contains(df.bondid) select df.payamount).DefaultIfEmpty(0).Sum();
            var bookingy = (from df in db.appltabs where df.formdate < stdate && df.type != "Adjustment" && tbd.Contains(df.bondid) && df.opid == pr.OperatorId select df.bookingamount).DefaultIfEmpty(0).Sum();
            var advancey = (from df in db.appltabs where df.formdate < stdate && df.type != "Adjustment" && tbd.Contains(df.bondid) && df.opid == pr.OperatorId select df.downpayment).DefaultIfEmpty(0).Sum();
            var todayNewBondcoll = todayfdcoll + todayRdcoll;
            var todayRenewalBondcoll = (from df in db.Installmenttabs where df.paymentdate < stdate && df.installmentno != 1 && df.type != "Adjustment" && df.opid == pr.OperatorId && tbd.Contains(df.bondid) select df.payamount).DefaultIfEmpty(0).Sum();

            var yeslatefee = (from df in db.Installmenttabs where df.paymentdate < stdate && df.type != "Adjustment" && df.opid == pr.OperatorId && tbd.Contains(df.bondid) select df.latefine).DefaultIfEmpty(0).Sum();
            var yesreleif = (from df in db.Installmenttabs where df.paymentdate < stdate && df.type != "Adjustment" && df.opid == pr.OperatorId && tbd.Contains(df.bondid) select df.reliefrs).DefaultIfEmpty(0).Sum();
            var ylatefee = yeslatefee - yesreleif;

            var bformfeey = (from df in db.AgentDetails where df.Doj < stdate && df.operatorid == pr.OperatorId select df.formfee).DefaultIfEmpty(0).Sum();
            var cformfeey = (from df in db.appltabs where df.formdate < stdate && df.type != "Adjustment" && df.opid == pr.OperatorId && tbd.Contains(df.bondid) select df.applicationcharge).DefaultIfEmpty(0).Sum();
            var mformfeey = (from df in db.Member_tabs where df.Cdate < stdate && df.Opid == pr.OperatorId select df.Fee).DefaultIfEmpty(0).Sum();
            var acformfeey = (from df in db.SavingAccountInfos where df.Doj < stdate && df.opid == pr.OperatorId select df.formfee).DefaultIfEmpty(0).Sum();
            var deposity = (from df in db.TransactionTabs where df.pdate < stdate && df.opid == pr.OperatorId select df.credit).DefaultIfEmpty(0).Sum();
            var intotaly = Math.Round((todayRenewalBondcoll + bformfeey + cformfeey + mformfeey + acformfeey + deposity + ylatefee + bookingy + advancey), 2);
            ...
            var AdvPayment = (from df in db.AdvBrokerPaymentTabs where df.date < stdate && df.opid == pr.OperatorId select df.amount).DefaultIfEmpty(0).Sum();
            var withdrawy = (from df in db.TransactionTabs where df.pdate < stdate && df.opid == pr.OperatorId select df.debit).DefaultIfEmpty(0).Sum();
            var expansey = (from df in db.Expenses where df.date_time < stdate && df.opid == pr.OperatorId select df.amount).DefaultIfEmpty(0).Sum();
            var todayVouchercoll = (from df in db.Voucher_Reports where df.date < stdate && df.Operatorid == pr.OperatorId select df.netamount).DefaultIfEmpty(0).Sum();
            var mat = (from df in db.MaturityTabs where df.maturitydate < stdate && df.opid == pr.OperatorId select df.maturityamount).DefaultIfEmpty(0).Sum();
            ...
            /////Today's Collection /////
            var todayfdcoll1 = (from df in db.Installmenttabs where df.paymentdate >= stdate && df.paymentdate <= enddate && df.mode == "Single" && df.type != "Adjustment" && df.opid == pr.OperatorId && tbd.Contains(df.bondid) select df.payamount).DefaultIfEmpty(0).Sum();
            var todayRdcoll1 = (from df in db.Installmenttabs where df.paymentdate >= stdate && df.paymentdate <= enddate && df.mode != "Single" && df.installmentno == 1 && df.type != "Adjustment" && df.opid == pr.OperatorId && df.planname == plan1.shortcutname && tbd.Contains(df.bondid) select df.payamount).DefaultIfEmpty(0).Sum();
            var todaybooking = (from df in db.appltabs where df.formdate >= stdate && df.formdate <= enddate && df.type != "Adjustment" && tbd.Contains(df.bondid) && df.opid == pr.OperatorId select df.bookingamount).DefaultIfEmpty(0).Sum();
            var todayadvance = (from df in db.appltabs where df.formdate >= stdate && df.formdate <= enddate && df.type != "Adjustment" && tbd.Contains(df.bondid) && df.opid == pr.OperatorId select df.downpayment).DefaultIfEmpty(0).Sum();
            var todayNewBondcoll1 = todayfdcoll1 + todayRdcoll1;
            var todayRenewalBondcoll1 = (from df in db.Installmenttabs where df.paymentdate >= stdate && df.paymentdate <= enddate && df.installmentno != 1 && df.opid == pr.OperatorId && df.type != "Adjustment" && tbd.Contains(df.bondid) select df.payamount).DefaultIfEmpty(0).Sum();

            var todaylatefee = (from df in db.Installmenttabs where df.paymentdate >= stdate && df.paymentdate <= enddate && df.type != "Adjustment" && df.opid == pr.OperatorId && tbd.Contains(df.bondid) select df.latefine).DefaultIfEmpty(0).Sum();
            var todayreleif = (from df in db.Installmenttabs where df.paymentdate >= stdate && df.paymentdate <= enddate && df.type != "Adjustment" && df.opid == pr.OperatorId && tbd.Contains(df.bondid) select df.reliefrs).DefaultIfEmpty(0).Sum();
            var latefee = todaylatefee - todayreleif;

            var bformfee = (from df in db.AgentDetails where df.Doj >= stdate && df.Doj <= enddate && df.operatorid == pr.OperatorId select df.formfee).DefaultIfEmpty(0).Sum();
            var cformfee = (from df in db.appltabs where df.formdate >= stdate && df.formdate <= enddate && df.type != "Adjustment" && df.opid == pr.OperatorId && tbd.Contains(df.bondid) select df.applicationcharge).DefaultIfEmpty(0).Sum();
            var mformfee = (from df in db.Member_tabs where df.Cdate >= stdate && df.Cdate <= enddate && df.Opid == pr.OperatorId select df.Fee).DefaultIfEmpty(0).Sum();
            var acformfee = (from df in db.SavingAccountInfos where df.Doj >= stdate && df.Doj <= enddate && df.opid == pr.OperatorId select df.formfee).DefaultIfEmpty(0).Sum();
            var deposit = (from df in db.TransactionTabs where df.pdate >= stdate && df.pdate <= enddate && df.opid == pr.OperatorId select df.credit).DefaultIfEmpty(0).Sum();
            var intotal = Math.Round((todayRenewalBondcoll1 + bformfee + cformfee + mformfee + acformfee + deposit + latefee + todaybooking + todayadvance), 2);
            ...
            var AdvPayment1 = (from df in db.AdvBrokerPaymentTabs where df.date >= stdate && df.date <= enddate && df.opid == pr.OperatorId select df.amount).DefaultIfEmpty(0).Sum();
            var withdraw = (from df in db.TransactionTabs where df.pdate >= stdate && df.pdate <= enddate && df.opid == pr.OperatorId select df.debit).DefaultIfEmpty(0).Sum();
            var expanse = (from df in db.Expenses where df.date_time >= stdate && df.date_time <= enddate && df.opid == pr.OperatorId select df.amount).DefaultIfEmpty(0).Sum();
            var todayVouchercoll1 = (from df in db.Voucher_Reports where df.date >= stdate && df.date <= enddate && df.Operatorid == pr.OperatorId select df.netamount).DefaultIfEmpty(0).Sum();
            var mat1 = (from df in db.MaturityTabs where df.maturitydate >= stdate && df.maturitydate <= enddate && df.opid == pr.OperatorId select df.maturityamount).DefaultIfEmpty(0).Sum();
            ...
            else
            {
                ...
                if (newagentid != null && newagentid != "")
                {
                    switch (filter)
                    {
                        ...
                        case 2:
                            Double fee1 = 0;
                            var tbd2 = (from ap in db.appltabs where ap.newintroducerid == newagentid && ap.status == 1 select ap.bondid);
                            appsumB = (from ag in db.appltabs where ag.formdate == date && ag.newintroducerid == newagentid && ag.opid == pr.OperatorId && ag.status == 1 select ag.applicationcharge).DefaultIfEmpty(0).Sum();
                            formsumB = (from ag in db.AgentDetails where ag.Doj == date && ag.NewAgentId == newagentid && ag.operatorid == pr.OperatorId select ag.formfee).DefaultIfEmpty(0).Sum();
                        ...
                        case 3:
                            if (year > 0)
                            {
                                var tbd3 = (from ap in db.appltabs where ap.newintroducerid == newagentid && ap.status == 1 select ap.bondid);
        ...
        public ActionResult FundMobilisation(DateTime sdate, DateTime edate)
        {
            ...
            else
            {
                ...
                var tbd = (from ap in db.appltabs where ap.status == 1 select ap.bondid);
                var planlist = (from pl in db.Installmenttabs where pl.paymentdate >= sdate && pl.paymentdate <= edate && pl.opid == pr.OperatorId && tbd.Contains(pl.bondid) select new { pl.planname }).Distinct();
                Double tsis = 0, tMISFresh = 0, tMIS1 = 0, tMIS2 = 0, tMIS3 = 0, tMIS4 = 0, tMIS5 = 0, tMIS6 = 0, tMIS7a = 0, total = 0, tpps = 0, tnfp = 0;
                foreach (var plan in planlist)
                {
                    tMISFresh = (from o in db.Installmenttabs where o.paymentdate >= sdate && o.paymentdate <= edate && o.planname == plan.planname && o.installmentno == 1 && o.mode != "Single" && o.plantype == "RD" && o.opid == pr.OperatorId && tbd.Contains(o.bondid) select o.payamount).DefaultIfEmpty(0).Sum();
                    tnfp = (from o in db.Installmenttabs where o.paymentdate >= sdate && o.paymentdate <= edate && o.planname == plan.planname && o.installmentno == 1 && o.mode != "Single" && o.plantype == "NFS" && o.opid == pr.OperatorId && tbd.Contains(o.bondid) select o.payamount).DefaultIfEmpty(0).Sum();
                    tsis = (from o in db.Installmenttabs where o.paymentdate >= sdate && o.paymentdate <= edate && o.planname == plan.planname && o.installmentno == 1 && o.mode == "Single" && o.plantype == "FD" && o.opid == pr.OperatorId && tbd.Contains(o.bondid) select o.payamount).DefaultIfEmpty(0).Sum();
                    tpps = (from o in db.appltabs where o.formdate >= sdate && o.formdate <= edate && o.planname == plan.planname && o.mode != "Single" && o.plantype == "PPS" && o.opid == pr.OperatorId && tbd.Contains(o.bondid) select o.totalcon).DefaultIfEmpty(0).Sum();
                    tMIS1 = (from o in db.Installmenttabs where o.paymentdate >= sdate && o.paymentdate <= edate && o.planname == plan.planname && o.installmentno != 1 && o.year == 1 && o.plantype == "RD" && o.opid == pr.OperatorId && tbd.Contains(o.bondid) select o.payamount).DefaultIfEmpty(0).Sum();
                    tMIS2 = (from o in db.Installmenttabs where o.paymentdate >= sdate && o.paymentdate <= edate && o.planname == plan.planname && o.installmentno != 1 && o.year == 2 && o.plantype == "RD" && o.opid == pr.OperatorId && tbd.Contains(o.bondid) select o.payamount).DefaultIfEmpty(0).Sum();
                    tMIS3 = (from o in db.Installmenttabs where o.paymentdate >= sdate && o.paymentdate <= edate && o.planname == plan.planname && o.installmentno != 1 && o.year == 3 && o.plantype == "RD" && o.opid == pr.OperatorId && tbd.Contains(o.bondid) select o.payamount).DefaultIfEmpty(0).Sum();
                    tMIS4 = (from o in db.Installmenttabs where o.paymentdate >= sdate && o.paymentdate <= edate && o.planname == plan.planname && o.installmentno != 1 && o.year == 4 && o.plantype == "RD" && o.opid == pr.OperatorId && tbd.Contains(o.bondid) select o.payamount).DefaultIfEmpty(0).Sum();
                    tMIS5 = (from o in db.Installmenttabs where o.paymentdate >= sdate && o.paymentdate <= edate && o.planname == plan.planname && o.installmentno != 1 && o.year == 5 && o.plantype == "RD" && o.opid == pr.OperatorId && tbd.Contains(o.bondid) select o.payamount).DefaultIfEmpty(0).Sum();
                    tMIS6 = (from o in db.Installmenttabs where o.paymentdate >= sdate && o.paymentdate <= edate && o.planname == plan.planname && o.installmentno != 1 && o.year == 6 && o.plantype == "RD" && o.opid == pr.OperatorId && tbd.Contains(o.bondid) select o.payamount).DefaultIfEmpty(0).Sum();
                    tMIS7a = (from o in db.Installmenttabs where o.paymentdate >= sdate && o.paymentdate <= edate && o.planname == plan.planname && o.installmentno != 1 && o.year >= 7 && o.plantype == "RD" && o.opid == pr.OperatorId && tbd.Contains(o.bondid) select o.payamount).DefaultIfEmpty(0).Sum();
        ...
        public ActionResult PrintFundMobilisation()
        {
            ...
            else
            {
                ...
                var tbd = (from ap in db.appltabs where ap.status == 1 select ap.bondid);
                var planlist = (from pl in db.Installmenttabs where pl.paymentdate >= stdate && pl.paymentdate <= enddate && pl.opid == pr.OperatorId && tbd.Contains(pl.bondid) select new { pl.planname }).Distinct();
                Double tsis = 0, tMISFresh = 0, tMIS1 = 0, tMIS2 = 0, tMIS3 = 0, tMIS4 = 0, tMIS5 = 0, tMIS6 = 0, tMIS7a = 0, total = 0, tpps = 0, tnfp = 0;
                foreach (var plan in planlist)
                {
                    tMISFresh = (from o in db.Installmenttabs where o.paymentdate >= stdate && o.paymentdate <= enddate && o.planname == plan.planname && o.installmentno == 1 && o.mode != "Single" && o.plantype == "RD" && o.opid == pr.OperatorId && tbd.Contains(o.bondid) select o.payamount).DefaultIfEmpty(0).Sum();
                    tnfp = (from o in db.Installmenttabs where o.paymentdate >= stdate && o.paymentdate <= enddate && o.planname == plan.planname && o.installmentno == 1 && o.mode != "Single" && o.plantype == "NFS" && o.opid == pr.OperatorId && tbd.Contains(o.bondid) select o.payamount).DefaultIfEmpty(0).Sum();
                    tsis = (from o in db.Installmenttabs where o.paymentdate >= stdate && o.paymentdate <= enddate && o.planname == plan.planname && o.installmentno == 1 && o.mode == "Single" && o.plantype == "FD" && o.opid == pr.OperatorId && tbd.Contains(o.bondid) select o.payamount).DefaultIfEmpty(0).Sum();
                    tpps = (from o in db.appltabs where o.formdate >= stdate && o.formdate <= enddate && o.planname == plan.planname && o.mode != "Single" && o.plantype == "PPS" && o.opid == pr.OperatorId && tbd.Contains(o.bondid) select o.totalcon).DefaultIfEmpty(0).Sum();
                    tMIS1 = (from o in db.Installmenttabs where o.paymentdate >= stdate && o.paymentdate <= enddate && o.planname == plan.planname && o.installmentno != 1 && o.year == 1 && o.plantype == "RD" && o.opid == pr.OperatorId && tbd.Contains(o.bondid) select o.payamount).DefaultIfEmpty(0).Sum();
                    tMIS2 = (from o in db.Installmenttabs where o.paymentdate >= stdate && o.paymentdate <= enddate && o.planname == plan.planname && o.installmentno != 1 && o.year == 2 && o.plantype == "RD" && o.opid == pr.OperatorId && tbd.Contains(o.bondid) select o.payamount).DefaultIfEmpty(0).Sum();
                    tMIS3 = (from o in db.Installmenttabs where o.paymentdate >= stdate && o.paymentdate <= enddate && o.planname == plan.planname && o.installmentno != 1 && o.year == 3 && o.plantype == "RD" && o.opid == pr.OperatorId && tbd.Contains(o.bondid) select o.payamount).DefaultIfEmpty(0).Sum();
                    tMIS4 = (from o in db.Installmenttabs where o.paymentdate >= stdate && o.paymentdate <= enddate && o.planname == plan.planname && o.installmentno != 1 && o.year == 4 && o.plantype == "RD" && o.opid == pr.OperatorId && tbd.Contains(o.bondid) select o.payamount).DefaultIfEmpty(0).Sum();
                    tMIS5 = (from o in db.Installmenttabs where o.paymentdate >= stdate && o.paymentdate <= enddate && o.planname == plan.planname && o.installmentno != 1 && o.year == 5 && o.plantype == "RD" && o.opid == pr.OperatorId && tbd.Contains(o.bondid) select o.payamount).DefaultIfEmpty(0).Sum();
                    tMIS6 = (from o in db.Installmenttabs where o.paymentdate >= stdate && o.paymentdate <= enddate && o.planname == plan.planname && o.installmentno != 1 && o.year == 6 && o.plantype == "RD" && o.opid == pr.OperatorId && tbd.Contains(o.bondid) select o.payamount).DefaultIfEmpty(0).Sum();
                    tMIS7a = (from o in db.Installmenttabs where o.paymentdate >= stdate && o.paymentdate <= enddate && o.planname == plan.planname && o.installmentno != 1 && o.year >= 7 && o.plantype == "RD" && o.opid == pr.OperatorId && tbd.Contains(o.bondid) select o.payamount).DefaultIfEmpty(0).Sum();
                    total = tMISFresh + tsis + tpps + tnfp + tMIS1 + tMIS2 + tMIS3 + tMIS4 + tMIS5 + tMIS6 + tMIS7a;
                    if (total > 0)
                    {
            ...
            else
            {
                ...
                if (tempcount == 0 && count == 0)
                {
                    ...
                    if (ddt.transfertype != "select")
                    {
                        ...
                        if (remailning > 0)
                        {
                            var max = (from a in db.ClosingAmount_Tabs where a.opid == User.Identity.Name select a.Trid).DefaultIfEmpty().Max() + 1;
        ...
        public ActionResult DailyClosingReport(DateTime sdate, DateTime edate)
        {
            ...
            {
                //DateTime ssdate = DateTime.ParseExact(sdate, "dd/MM/yyyy", null);
                //DateTime eedate = DateTime.ParseExact(edate, "dd/MM/yyyy", null);
